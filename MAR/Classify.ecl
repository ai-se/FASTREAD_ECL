...
*/
  ...
  EXPORT SVM(SVM.Types.SVM_Type svm_type=0, SVM.Types.Kernel_Type kernel_type=0,
             INTEGER4 degree=3, REAL8 gamma=0.1, REAL8 coef0=0, REAL8 cost=1, REAL8 eps=0.001,
             REAL8 nu=0.5, REAL8 p=0.1, BOOLEAN shrinking=TRUE) := MODULE(DEFAULT)
    ...
    EXPORT ClassifyC(DATASET(Types.NumericField) Indep,
      ...
      pred := SVM.predict(mdl, inst_data).Pred_Prob_One;
      // convert to standard form
      l_result cvt(SVM.Types.SVM_Pred_Prob_One p) := TRANSFORM
        SELF.id := p.rid;
        SELF.number := p.id; // model ID is the dependent var field ID
        SELF.value := p.predict_y;
        SELF.conf := p.prob_estimates; // confidence
      END;
